"""PromptGuard CLI - Command-line interface for LLM behavior testing."""
...
from promptguard.agent.loader import load_agent_bundle
from promptguard.agent.models import AgentRunResult
from promptguard.agent.runner import AgentRunner
...
from promptguard.reporters import get_reporter
...
@click.group()
@click.version_option(version=__version__, prog_name="promptguard")
def main() -> None:
    """PromptGuard - CI/CD for LLM behavior.
...
def init(path: str, force: bool) -> None:
    ...
    console.print(f"  2. Validate: [bold]promptguard validate {path}/bundle.yaml[/bold]")
    console.print(f"  3. Run: [bold]promptguard run {path}/bundle.yaml[/bold]")
...
def promote(
    ...
    Executes all test cases and saves the results to .promptguard/baselines/
    ...
    try:
        ...
        from promptguard.baseline.models import BaselineStorage

        timestamp_str = baseline_run.timestamp.strftime("%Y%m%d_%H%M%S")
        baseline_dir = Path(".promptguard") / "baselines" / bundle_name
...
def agent_init(path: str, force: bool) -> None:
    ...
    console.print(f"  2. Validate: [bold]promptguard agent validate {path}/[/bold]")
    console.print(f"  3. Run: [bold]promptguard agent run {path}/[/bold]")
...
def agent_run(
    ...
    try:
        ...
        # Evaluate results
        from promptguard.agent.evaluator import AgentEvaluator
...
def diff(
    ...
    """
    try:
        from promptguard.bundle.loader import BundleLoader
        from promptguard.diff import Comparator
        from promptguard.baseline.storage import BaselineStorage
        from pathlib import Path
        ...
        storage_backend = BaselineStorageBackend(".promptguard/baselines")
        ...
        # Run current bundle
        from promptguard.runner.engine import Runner
        from promptguard.providers import get_provider
